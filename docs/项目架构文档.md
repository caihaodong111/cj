...
flowchart TB
    subgraph Entry["入口层"]
        main["backend/crawler/main.py<br/>程序入口"]
...
```mermaid
sequenceDiagram
    participant Main as backend/crawler/main.py
...
2. 实现以下核心文件：
   ...
   - `field.py` - 定义平台枚举
3. 在 `store/` 下创建对应存储目录
4. 在 `backend/crawler/main.py` 的 `CrawlerFactory.CRAWLERS` 中注册
...
# 指定平台
python backend/crawler/main.py --platform xhs

# 指定登录方式
python backend/crawler/main.py --lt qrcode

# 指定爬虫类型
python backend/crawler/main.py --type search
```
